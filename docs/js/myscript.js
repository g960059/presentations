$(window).on('load', function(){
  $(".fg-10").attr('data-fragment-index', 10).addClass("fragment");
  $(".fg-20").attr('data-fragment-index', 20).addClass("fragment");
  $(".fg-30").attr('data-fragment-index', 30).addClass("fragment");
  $(".fg-40").attr('data-fragment-index', 40).addClass("fragment");
  $(".fg-50").attr('data-fragment-index', 50).addClass("fragment");
  $(".fg-60").attr('data-fragment-index', 60).addClass("fragment");
  $(".fg-1").attr('data-fragment-index', 1).addClass("fragment");
  $(".fg-2").attr('data-fragment-index', 2).addClass("fragment");
  $(".fg-3").attr('data-fragment-index', 3).addClass("fragment");
  $(".fg-4").attr('data-fragment-index', 4).addClass("fragment");
  $(".fg-5").attr('data-fragment-index', 5).addClass("fragment");
  $(".fg-6").attr('data-fragment-index', 6).addClass("fragment");
  $(".fg-7").attr('data-fragment-index', 7).addClass("fragment");  
  $(".fg-8").attr('data-fragment-index', 8).addClass("fragment");   
  $(".fg-9").attr('data-fragment-index', 9).addClass("fragment");  
  $(".fg").addClass("fragment");
});
